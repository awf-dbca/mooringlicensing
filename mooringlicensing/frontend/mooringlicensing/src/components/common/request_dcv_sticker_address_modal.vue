...
export default {
    ...
    watch: {
        dcv_permit_id: async function(){
            let vm = this
            // Whenever dcv_permit_id is changed, update this.stickers
            if (vm.dcv_permit_id){
                const ret = await utils.fetchUrl(helpers.add_endpoint_json('/api/internal_dcv_permit/', vm.dcv_permit_id + '/stickers'))
